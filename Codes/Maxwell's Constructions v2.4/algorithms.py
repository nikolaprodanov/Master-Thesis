...
def find_global_minima_free_energy(delta, mu_tilde, N_modes):
    ...
    free_energy = [fun.Free_Helmholtz_Energy(delta, mu_tilde, x, N_modes) for x in x_grid]

    # Find index of minimum directly (no min() + index())
    min_index = np.argmin(free_energy)
    return x_grid[min_index]
...
def construct_data_mutilde_n_x(delta, N_modes):
    ...
    for mu_tilde in mu_tilde_array:
        #print(f'Other mu_tilde = {mu_tilde}')
    ...
    #print('Im exiting the found solutions')
