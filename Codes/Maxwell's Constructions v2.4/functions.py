...
from constants import lambd, x_min, x_max, epsilon
import math
import numpy as np
...
def density(delta, mu_tilde, x, N_modes):
    ...
    if mu_tilde == 0: # add also mu_tilde = 2
        ...
        if delta == 0:
            ...
            # Method 2
            #sum_nu_pos, sum_nu_neg = 0.0, 0.0
            #for k_i in k:
            #    sum_nu_neg = sum_nu_neg + my_sign( np.sqrt(np.cos(k_i)**2 + (x/2)**2) + (+1)*mu_tilde/2 )
            #    sum_nu_pos = sum_nu_pos + my_sign( np.sqrt(np.cos(k_i)**2 + (x/2)**2) + (-1)*mu_tilde/2 )
            
            # Method 3
            common_term = np.sqrt(np.cos(k)**2 + (x / 2)**2)
            sum_nu_neg = np.sum(np.sign(common_term + mu_tilde / 2))
            sum_nu_pos = np.sum(np.sign(common_term - mu_tilde / 2))
...
def derivative_Free_Helmholtz_Energy(delta, mu_tilde, x, N_modes): # this one is actually wrong !!!!
    ...
    if delta == 0:
        if mu_tilde == 0:
            ...
            sum_nu_pos = np.sum(np.sign(np.sqrt((np.cos(k))**2+(x/2)**2) + (mu/2)) * (x/2)/(np.sqrt((np.cos(k))**2+(x/2)**2))) 
            sum_nu_neg = np.sum(np.sign(np.sqrt((np.cos(k))**2+(x/2)**2) - (mu/2)) * (x/2)/(np.sqrt((np.cos(k))**2+(x/2)**2))) 
